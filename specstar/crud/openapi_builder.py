"""OpenAPI schema customisation for SpecStar.

All OpenAPI-specific logic that was previously spread across SpecStar instance
methods lives here.  Callers construct an ``OpenAPIBuilder`` with the relevant
SpecStar state and call ``customize(app, structs)``.
"""
...
class OpenAPIBuilder:
    """Encapsulates all OpenAPI schema customisation for an SpecStar instance."""
    ...
    def _inject_ref_metadata(self, schema: dict) -> None:
        ...
        def _process_single_struct(
            ...
            *,
            ...
            from specstar.types import extract_display_name
        ...
        if all_refs:
            schema["x-specstar-relationships"] = [
                {
                    "source": r.source,
    ...
    def _inject_custom_create_actions(self, schema: dict) -> None:
        """Inject ``x-specstar-custom-create-actions`` top-level extension."""
        ...
        if actions_by_resource:
            schema["x-specstar-custom-create-actions"] = dict(actions_by_resource)

    def _inject_custom_update_actions(self, schema: dict) -> None:
        """Inject ``x-specstar-custom-update-actions`` top-level extension."""
        ...
        if actions_by_resource:
            schema["x-specstar-custom-update-actions"] = dict(actions_by_resource)

    def _inject_async_create_jobs(self, schema: dict) -> None:
        """Inject ``x-specstar-async-create-jobs`` top-level extension."""
        ...
        if mapping:
            schema["x-specstar-async-create-jobs"] = mapping

    def _inject_async_update_jobs(self, schema: dict) -> None:
        """Inject ``x-specstar-async-update-jobs`` top-level extension."""
        ...
        if mapping:
            schema["x-specstar-async-update-jobs"] = mapping

    def _inject_indexed_fields(self, schema: dict) -> None:
        """Inject ``x-specstar-indexed-fields`` top-level extension."""
        ...
        if mapping:
            schema["x-specstar-indexed-fields"] = mapping
    ...
    def _get_unique_fields(rm: Any) -> list[str]:
        ...
        from specstar.resource_manager.constraint_handler import ConstraintEventHandler
    ...
    def _inline_embedded_schema_ref(schema_extra: dict, source_type: Any) -> dict:
        ...
        try:
            ...
            from specstar.crud.route_templates.basic import jsonschema_to_openapi
