...
Both :class:`~specstar.permission.acl.ACLPermissionChecker` and
:class:`~specstar.permission.rbac.RBACPermissionChecker` share the same
...
* :class:`Policy` — the policy flag (lives here so it can be reused without
  pulling :mod:`specstar.permission.acl`).
