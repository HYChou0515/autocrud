...
def build_from_config():
    ...
    module_name = "specstar_cli_models"
...
class ResourceUI:
    ...
    def create(self):
        ...
        resp = httpx.post(
            f"{self.user_config.specstar_url}/{self.name}",
            json=json.loads(obj.model_dump_json()),
        )
        resp.raise_for_status()
        resource_id = resp.json()["resource_id"]
        resp = self.retry(httpx.get)(
            f"{self.user_config.specstar_url}/{self.name}/{resource_id}/full",
        )
        resp.raise_for_status()
    ...
    def update(self, page: PageOptions):
        ...
        resp = httpx.put(
            f"{self.user_config.specstar_url}/{self.name}/{obj.meta.resource_id}",
    ...
    def page_objects(
        ...
        resp = self.retry(httpx.get)(
            f"{self.user_config.specstar_url}/{self.name}/full",
            params={
