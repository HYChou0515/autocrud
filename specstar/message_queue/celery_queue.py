...
class CeleryMessageQueue(DelayableMessageQueue[T], Generic[T]):
    ...
    while using SpecStar ResourceManager for payload storage and status persistence.
    ...
    Example:
        ...
        celery_app = Celery("specstar", broker="redis://localhost:6379/0")
    ...
    def __init__(
        ...
        Note:
            Each :class:`~specstar.types.Job` may set its own ``max_retries``.
        ...
        if queue_name is None:
            queue_name = f"specstar.{resource_manager.resource_name}"
...
class CeleryMessageQueueFactory:
    ...
    def __init__(
        ...
        Args:
            celery_app: Celery application instance.
            queue_prefix: Prefix for queue names (default: "specstar.").
