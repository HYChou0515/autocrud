...
def process_file(filepath):
    ...
    content = re.sub(r"^```\{code-block\}\s*(\w+)?", r"```\1", content, flags=re.MULTILINE)
    ...
    content = re.sub(r"```\{versionadded\}\s*(.*?)\n(.*?)```", repl_version_added, content, flags=re.DOTALL)
    ...
    content = re.sub(r"```\{versionchanged\}\s*(.*?)\n(.*?)```", repl_version_changed, content, flags=re.DOTALL)
    ...
    content = re.sub(r"```\{include\}\s*(\S+)\s*\n```", r'--8<-- "docs/includes/\1"', content)
    ...
    content = re.sub(r"^```(\w*)\n:emphasize-lines:\s*(.+)$", repl_emphasize, content, flags=re.MULTILINE)
    ...
    def repl_autoclass(match):
        ...
        rel_to_root = os.path.relpath(os.path.join(DOCS_DIR, "reference"), os.path.dirname(filepath))
    ...
    admonition_types = ["attention", "caution", "danger", "error", "hint", "important", "note", "tip", "warning", "seealso"]
    ...
    def transform_admonitions(text):
        ...
        start_pattern = re.compile(r"^(`{3,})\{(" + "|".join(admonition_types) + r")\}\s*(.*)$", re.IGNORECASE)
    ...
    content = re.sub(pattern, repl_admonition, content, flags=re.MULTILINE | re.DOTALL | re.IGNORECASE)
    ...
    content = re.sub(r'<div class="mermaid">\s*(.*?)\s*</div>', repl_mermaid_div, content, flags=re.DOTALL)
