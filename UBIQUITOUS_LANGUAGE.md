...
| **Event Handler**     | A callable invoked by SpecStar when an **Event** fires.                                          | Listener, observer, callback   |
...
| Term                  | Definition                                                                                                    | Aliases to avoid                       |
| --------------------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| **Public Namespace**  | A curated import path SpecStar users are expected to import from (e.g. `specstar.events`, `specstar.errors`). | Public API, top-level import           |
...
- An **Event Handler** is invoked by SpecStar whenever its supported **Event Context** is emitted.
...
> **Dev:** "If I `move` `IEventHandler` into `specstar.events`, won't I lose the import path that uses `specstar.types`?"

> **Maintainer:** "Yes, but that path was a **Re-export Facade** with no **Curated Namespace** behind it. The **Canonical Owner** for anything event-shaped should be `specstar.events` — that's where the **Event Context** structs already live."

> **Dev:** "And the `__getattr__` at the bottom of `specstar.events`?"

> **Maintainer:** "That's a **Backward-Compat Shim** that exists to dodge a **Cycle** with `specstar.types`. Once we **Cycle Break** by moving `IEventHandler`, we delete it. No **Inline Import** needed."

> **Dev:** "What about callers that import `ResourceMetaSearchQuery` from `specstar.types`?"

> **Maintainer:** "Same idea — `specstar.query_types` is the **Canonical Owner**. `specstar.types` should only re-export it if we explicitly mark it as a **Curated Namespace** with `__all__`. Otherwise update the callers."

## Flagged ambiguities

- **"events"** has been used to mean three different things in this conversation: (1) the lifecycle **Event** concept, (2) the `specstar.events` **Public Namespace**, and (3) the `specstar.resource_manager.events` private implementation module. Use **Event** for the concept, and prefix with the dotted path when referring to a module.
...
- **"public API"** — used loosely to mean both the package's `__init__.py` exports and any `Public Namespace`. Prefer **Public Namespace** when discussing per-module surfaces, and "package root" or "top-level public API" when referring to `specstar/__init__.py`.
