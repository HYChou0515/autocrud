...
export type PetJobFieldName = 'payload' | 'status' | 'errmsg' | 'retries' | 'periodic_interval_seconds' | 'periodic_max_runs' | 'periodic_runs' | 'periodic_initial_delay_seconds' | 'last_heartbeat_at';
export type GameEventFieldName = 'payload.event_type' | 'payload.character_name' | 'payload.character_id' | 'payload.description' | 'payload.reward_gold' | 'payload.reward_exp' | 'payload.extra_data' | 'payload.event_body' | 'payload.event_x.type' | 'payload.event_x.good' | 'payload.event_x.great' | 'status' | 'errmsg' | 'retries' | 'periodic_interval_seconds' | 'periodic_max_runs' | 'periodic_runs' | 'periodic_initial_delay_seconds' | 'last_heartbeat_at';
...
Object.assign(registry, {
  'character': {
    ...
    f: z.object({
        skname: z.string(),
        detail: z.discriminatedUnion('type', [z.object({ type: z.literal('active'), mp_cost: z.number().int().optional(), cooldown_seconds: z.number().int().optional(), damage: z.number().int().optional() }), z.object({ type: z.literal('passive'), buff_percentage: z.number().int().optional() }), z.object({ type: z.literal('ultimate'), mp_cost: z.number().int().optional(), cooldown_seconds: z.number().int().optional(), damage: z.number().int().optional(), area_of_effect: z.boolean().optional() })]),
  ...
  'pet-job': {
    ...
    apiClient: petJobApi,
    maxFormDepth: 1,
    defaultHiddenFields: ["status","errmsg","retries","periodic_interval_seconds","periodic_max_runs","periodic_runs","periodic_initial_delay_seconds"],
  },
  'game-event': {
    name: 'game-event',
    label: 'Game Event',
