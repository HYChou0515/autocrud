 ...
 * @param spec - Raw OpenAPI spec (for x-specstar-async-create-jobs)
...
export function genResourcesConfig(
  ...
  const asyncJobsMap: Record<string, string> = spec['x-specstar-async-create-jobs'] ?? {};
  const asyncUpdateJobsMap: Record<string, string> = spec['x-specstar-async-update-jobs'] ?? {};
...
${displayNameLine}    schema: '${r.schemaName}',
  ...
  const detailRouteUnion = resources.map((r) => `'/specstar-admin/${r.name}/$resourceId'`).join('\n  | ');
  const listRouteUnion = resources.map((r) => `'/specstar-admin/${r.name}'`).join('\n  | ');
  const detailRouteCases = resources.map((r) => `  '${r.name}': '/specstar-admin/${r.name}/$resourceId'`).join(',\n');
  const listRouteCases = resources.map((r) => `  '${r.name}': '/specstar-admin/${r.name}'`).join(',\n');
...
// include the correct prefix (e.g. '/v1/specstar/blobs/upload').
