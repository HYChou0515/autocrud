...
${zodFields}
    ...
    return `// Auto-generated by AutoCRUD Web Generator
import { resources as registry } from '../lib/resources';
import { z } from 'zod';
${imports}
...
function DetailPage() {
  ...
  const config = getResource('${r.name}')!;
  return <ResourceDetail<${r.schemaName}> config={config} resourceId={resourceId} basePath="/autocrud-admin/${r.name}"${r.isJob ? ' isJob={true}' : ''} />;
}
`;
  }
