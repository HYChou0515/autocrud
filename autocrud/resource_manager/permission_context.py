...
class PermissionContext(Struct, kw_only=True):
    ...
    def set_resource_meta(self, meta: Any) -> None:
        """設置資源元資料（用於延遲加載）"""
        self._resource_meta = meta
    
    @property
    def resource_data(self) -> Any:
        """獲取資源資料"""
        return self._resource_data
    
    @property
    def resource_meta(self) -> Any:
        """獲取資源元資料"""
...
class ResourceOwnershipChecker(PermissionChecker):
    ...
    def __init__(self, resource_manager: 'ResourceManager', 
        ...
        self.allowed_actions = allowed_actions or {"update", "delete", "patch"}
    ...
    def check_permission(self, context: PermissionContext) -> PermissionResult:
        ...
        try:
            # 獲取資源元資料
            if context.resource_meta is None:
                meta = self.resource_manager.get_meta(context.resource_id)
                context.set_resource_meta(meta)
            else:
...
class FieldLevelPermissionChecker(PermissionChecker):
    ...
    def __init__(self, allowed_fields_by_user: Dict[str, set[str]] = None,
...
class ConditionalPermissionChecker(PermissionChecker):
    ...
    def check_permission(self, context: PermissionContext) -> PermissionResult:
        ...
        return PermissionResult.ALLOW if self._conditions else PermissionResult.NOT_APPLICABLE
...
def create_default_permission_checker(
    ...
    if field_permissions:
        checkers.append(FieldLevelPermissionChecker(allowed_fields_by_user=field_permissions))
