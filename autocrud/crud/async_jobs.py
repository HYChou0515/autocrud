"""Async job model registration for AutoCRUD create / update actions.

Functions here are called by AutoCRUD during ``apply()`` to auto-register
Job models for ``async_mode='job'`` actions.  They take the relevant AutoCRUD
state as plain arguments and return the registry dicts that are stored back
on the AutoCRUD instance.
"""
...
def register_async_create_jobs(
    ...
    from autocrud.util.type_utils import unwrap_annotated
