...
class TestCaseUtil:
    ...
    def setup_permission_system(self):
        ...
        storage = SimpleStorage(meta_store=MemoryMetaStore(), resource_store=resource_store)
        
        # 設定權限管理器
        permission_resource_store = MemoryResourceStore(dict)
        permission_storage = SimpleStorage(meta_store=MemoryMetaStore(), resource_store=permission_resource_store)
    ...
    def test_create_permissions(self):
        ...
        with pm.meta_provide("system", current_time):
            ...
            alice_permissions = ACLPermission(
                ...
                action=ResourceAction.create | ResourceAction.read | ResourceAction.update | ResourceAction.delete,
                effect=Effect.allow
            )
...
class TestRootUserPermissions(TestCaseUtil):
    ...
    def test_root_user_has_all_permissions(self):
        ...
        with pm.meta_provide("root", current_time):
            # Root 應該有所有權限
            assert pm.check_permission("root", ResourceAction.create, "test_document") == True
            assert pm.check_permission("root", ResourceAction.read, "test_document") == True
            assert pm.check_permission("root", ResourceAction.update, "test_document") == True
            assert pm.check_permission("root", ResourceAction.delete, "test_document") == True
            assert pm.check_permission("root", ResourceAction.full, "any_resource") == True
...
class TestResourceManagerCRUDOperations(TestCaseUtil):
    ...
    def setup_permissions(self):
        ...
        with pm.meta_provide("system", current_time):
            ...
            alice_permissions = ACLPermission(
                subject="alice",
                object="test_document",
                action=ResourceAction.create | ResourceAction.read | ResourceAction.update | ResourceAction.delete | ResourceAction.read_list,
                effect=Effect.allow
            )
    ...
    def test_update_operation(self):
        ...
        with pytest.raises(PermissionDeniedError):
            with dm.meta_provide("bob", current_time):
                updated_doc = TestDocument(title="Bob's Update", content="Bob's Content")
                dm.update(doc_id, updated_doc)
        
        # Charlie 不能更新（只有創建權限）
        with pytest.raises(PermissionDeniedError):
            with dm.meta_provide("charlie", current_time):
                updated_doc = TestDocument(title="Charlie's Update", content="Charlie's Content")
                dm.update(doc_id, updated_doc)
...
class TestRootUserOperations(TestCaseUtil):
    ...
    def test_root_can_perform_all_operations(self):
        ...
        with dm.meta_provide("root", current_time):
            ...
            # Root 可以更新
            updated_doc = TestDocument(title="Root's Updated Doc", content="Root's Updated Content")
            update_info = dm.update(doc_id, updated_doc)
            assert update_info.resource_id == doc_id
    ...
    def test_no_permissions_user(self):
        ...
        with pytest.raises(PermissionDeniedError):
            with dm.meta_provide("unauthorized", current_time):
                doc = TestDocument(title="Unauthorized Doc", content="Unauthorized Content")
                dm.create(doc)
...
class TestComplexPermissionScenarios(TestCaseUtil):
    ...
    def test_permission_isolation_between_resource_types(self):
        ...
        with pm.meta_provide("specialized_user", current_time):
            ...
            assert pm.check_permission("specialized_user", ResourceAction.create, "other_resource") == True
            assert pm.check_permission("specialized_user", ResourceAction.read, "other_resource") == True
            
            # 對 test_document 沒有權限
            assert pm.check_permission("specialized_user", ResourceAction.create, "test_document") == False
            assert pm.check_permission("specialized_user", ResourceAction.read, "test_document") == False
