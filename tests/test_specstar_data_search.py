...
class TestSpecStarDataSearch:
    """High-level tests for SpecStar data search functionality."""

    @pytest.fixture(autouse=True)
    def setup_method(self):
        """Setup SpecStar with User model and create test data."""
        # 創建 SpecStar 實例
        self.specstar = SpecStar(
            storage_factory=MemoryStorageFactory(),
        )

        # 添加 User 模型並指定索引字段
        self.specstar.add_model(
            User,
            name="users",
            indexed_fields=[
                ("name", str),
        ...
        # 獲取 resource manager
        self.user_manager: ResourceManager[User] = self.specstar.resource_managers[
            "users"
