...
def test_simple_delay_retry():
    """Test DelayRetry with SimpleMessageQueue."""
    execution_times = []
    execution_count = [0]
...
def test_simple_delay_retry_multiple_times():
...
class TestRabbitMQDelayRetry:
    """Tests for DelayRetry with RabbitMQ."""

    def test_delay_retry_basic(self, rm_fixture):
        """Test DelayRetry with RabbitMQMessageQueue."""
        execution_count = [0]
        execution_times = []
        ...
        # Create a job
        job = Job(
            payload=SimpleTask(name="rabbitmq_delay", value=123),
            status=TaskStatus.PENDING,
        )
    ...
    def test_delay_retry_different_intervals(self, rm_fixture):
...
def test_delay_retry_resets_retry_count():
