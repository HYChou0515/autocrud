...
def _put_chunk(client, upload_id, data, content_type="application/octet-stream"):
    return client.put(
...
class TestChunkedUploadRoutes:
    ...
    def test_multi_chunk_upload_and_finalize(self, client):
        ...
        # Upload chunk 1
        resp = _put_chunk(client, upload_id, CHUNK_1)
        assert resp.status_code == 200
        body = resp.json()
        ...
        # Upload chunk 2
        resp = _put_chunk(client, upload_id, CHUNK_2)
        assert resp.status_code == 200
        body = resp.json()
        assert body["status"] == "uploading"
        ...
        # Upload chunk 3
        resp = _put_chunk(client, upload_id, CHUNK_3)
        assert resp.status_code == 200
        body = resp.json()
        assert body["status"] == "uploading"
    ...
    def test_single_chunk_upload(self, client):
        ...
        resp = _put_chunk(client, upload_id, data)
        assert resp.status_code == 200
    ...
    def test_abort_during_chunked_upload(self, client):
        ...
        _put_chunk(client, upload_id, CHUNK_1)
        _put_chunk(client, upload_id, CHUNK_2)
    ...
    def test_chunked_upload_used_in_resource_create(self, client):
        ...
        _put_chunk(client, upload_id, CHUNK_1)
        _put_chunk(client, upload_id, CHUNK_2)
        _put_chunk(client, upload_id, CHUNK_3)
