...
- PostgreSQL: localhost:5432 (user: postgres, db: specstar_test)
...
def load_app(main_py_code: str, tmp_path: Path, config_name: str):
    ...
    # --- 1. Replace global `spec` with local SpecStar instance ---
    # The generated code uses `from specstar import ..., spec, ...`
    # We replace `spec` in-place so tests don't pollute global state.
    code = re.sub(
        r"from specstar import (.+)",
        _replace_crud_import,
        code,
        count=1,
    )
    # Insert `spec = SpecStar()` after the specstar import block
    code = re.sub(
        r"(from specstar(?:\.[\w.]+)? import [^\n]+\n)(?!spec = )",
    ...
    code = re.sub(
        ...
        f'\\1bucket="specstar-test-{s3_unique}"',
        code,
    )
...
def _replace_crud_import(match: re.Match) -> str:
    """Remove `spec` from the specstar import line, add SpecStar instead."""
    imports_str = match.group(1)
    items = [i.strip() for i in imports_str.split(",")]
    items = [i for i in items if i != "spec"]
    if "SpecStar" not in items:
        items.append("SpecStar")
    return f"from specstar import {', '.join(items)}"


def _insert_crud_instance(match: re.Match) -> str:
    """Insert `spec = SpecStar()` after the last specstar import line."""
    return match.group(0) + "spec = SpecStar()\n"
...
def _cleanup_s3(s3_unique: str) -> None:
    ...
    try:
        ...
        bucket = f"specstar-test-{s3_unique}"
        try:
...
WIZARD_CONFIGS: list[tuple[str, dict, set[str]]] = [
    ...
    (
        ...
        _make_overrides(
            ...
            encoding="msgpack",
            storage_config={
                "connectionString": "postgresql://postgres@localhost:5432/specstar_test",
                "s3Bucket": "specstar-test",
