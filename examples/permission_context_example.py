...
from autocrud.resource_manager.permission_context import (
    PermissionContext, PermissionResult, PermissionChecker,
    DefaultPermissionChecker, ActionBasedPermissionChecker, 
    ResourceOwnershipChecker, FieldLevelPermissionChecker,
    CompositePermissionChecker, ConditionalPermissionChecker,
    create_default_permission_checker, create_custom_action_checker
)
from autocrud.resource_manager.permission import PermissionResourceManager


# 示例 1: 基本使用 - 結合現有的 ACL/RBAC 系統
def setup_basic_permission_checker(permission_manager: PermissionResourceManager,
                                   resource_manager) -> PermissionChecker:
    """設置基本的權限檢查器"""
...
def setup_field_level_permissions() -> FieldLevelPermissionChecker:
    ...
    field_permissions = {
        ...
        "user:admin": {"name", "email", "description", "status", "priority"},  # admin 可以修改更多
    }
...
def usage_example():
    ...
    dynamic_checker.add_rule("no_weekend_delete", 
        lambda ctx: PermissionResult.DENY if ctx.action == "delete" and 
