...
def test_complete_permission_system():
    ...
    permission_manager = PermissionResourceManager(
        storage=permission_storage, name="permission"
    )
    ...
    with permission_manager.meta_provide("alice", current_time):
        alice_can_create = permission_manager.check_permission(
            "alice", ResourceAction.create, "test_document"
        )
        alice_can_read = permission_manager.check_permission(
            "alice", ResourceAction.read, "test_document"
        )
        alice_can_update = permission_manager.check_permission(
            "alice", ResourceAction.update, "test_document"
        )
    ...
    with permission_manager.meta_provide("bob", current_time):
        bob_can_create = permission_manager.check_permission(
            "bob", ResourceAction.create, "test_document"
        )
        bob_can_read = permission_manager.check_permission(
            "bob", ResourceAction.read, "test_document"
        )
        bob_can_update = permission_manager.check_permission(
            "bob", ResourceAction.update, "test_document"
        )
    ...
    with permission_manager.meta_provide("root", current_time):
        root_can_create = permission_manager.check_permission(
            "root", ResourceAction.create, "test_document"
        )
        root_can_read = permission_manager.check_permission(
            "root", ResourceAction.read, "test_document"
        )
        root_can_update = permission_manager.check_permission(
            "root", ResourceAction.update, "test_document"
        )
        root_can_delete = permission_manager.check_permission(
            "root", ResourceAction.delete, "test_document"
        )
