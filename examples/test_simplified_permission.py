...
def test_simplified():
    ...
    permission = builder.allow_user_on_resource_type("alice", "test_document", ["create", "read"])
    ...
    for key in meta_store:
        ...
        if 'action' in meta.indexed_data:
            print(f"  Action value: {meta.indexed_data['action']} (type: {type(meta.indexed_data['action'])})")
    
    # 測試權限檢查
    print("\n=== 測試權限檢查 ===")
    with permission_manager.meta_provide("user:alice", current_time):
        can_create = permission_manager.check_permission("user:alice", "create", "test_document")
